...
public class TerminalView extends View {
    ...
    int mTopRow;
    /** Sub-row vertical pixel offset in [0, mFontLineSpacing). */
    public float mSubRowOffsetPx = 0f;
    ...
    public int[] getColumnAndRow(MotionEvent event, boolean relativeToScroll) {
        ...
        int row = (int) ((event.getY() - mSubRowOffsetPx - mRenderer.mFontLineSpacingAndAscent) / mRenderer.mFontLineSpacing);
        if (relativeToScroll) {
            row += mTopRow;
        }
    ...
    protected void onDraw(Canvas canvas) {
        if (mEmulator == null) {
            ...
            mRenderer.render(mEmulator, canvas, mTopRow, mSubRowOffsetPx, sel[0], sel[1], sel[2], sel[3]);
    ...
    public int getCursorY(float y) {
        return (int) (((y - mSubRowOffsetPx - 40) / mRenderer.mFontLineSpacing) + mTopRow);
    }
    ...
    public int getPointY(int cy) {
        return Math.round((cy - mTopRow) * mRenderer.mFontLineSpacing + mSubRowOffsetPx);
    }
    ...
    public void setTopRow(int mTopRow) {
        ...
        this.mSubRowOffsetPx = 0f;
    }

    public void setSmoothScroll(int topRow, float subRowOffsetPx) {
        int min = mEmulator == null ? 0 : -mEmulator.getScreen().getActiveTranscriptRows();
        this.mTopRow = Math.min(0, Math.max(min, topRow));
        this.mSubRowOffsetPx = (this.mTopRow <= min) ? 0f : Math.max(0f, subRowOffsetPx);
        invalidate();
    }

    public float getSubRowOffsetPx() {
        return mSubRowOffsetPx;
    }
